...
impl FromStr for Angle {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let s = s.trim();
        if s.ends_with("rad") {
            let radians = s[..s.len() - 3].parse::<f32>().map_err(|_| PicturifyError::ParseError("Invalid angle".to_string()))?;
            Ok(Angle::Radians(radians))
        } else if s.ends_with("deg") {
            let degrees = s[..s.len() - 3].parse::<f32>().map_err(|_| PicturifyError::ParseError("Invalid angle".to_string()))?;
            Ok(Angle::Degrees(degrees))
        } else {
            let radians = s.parse::<f32>().map_err(|_| PicturifyError::ParseError("Invalid angle".to_string()))?;
            Ok(Angle::Radians(radians))
        }
    }
}