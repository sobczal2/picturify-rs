...
impl GrayscaleProcessor {
    ...
    fn luminosity_processing_function_fast() -> Box<dyn Fn(&mut Rgba<u8>, usize, usize) + Send + Sync> {
        Box::new(
            |pixel, _x, _y| {
                let avg = 0.21 * pixel.0[0] as f32 + 0.72 * pixel.0[1] as f32 + 0.07 * pixel.0[2] as f32;
                for i in 0..3 {
                    pixel.0[i] = avg as u8;
                }
            }
    ...
    fn lightness_processing_function() -> Box<dyn Fn(LinSrgba, usize, usize) -> LinSrgba + Send + Sync> {
        Box::new(
            |mut pixel, _x, _y| {
    ...
    fn luminosity_processing_function() -> Box<dyn Fn(LinSrgba, usize, usize) -> LinSrgba + Send + Sync> {
        Box::new(
            |mut pixel, _x, _y| {
