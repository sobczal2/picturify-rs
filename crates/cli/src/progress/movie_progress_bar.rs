...
pub fn run_progress_bar_for_movie(movie_progress: Arc<RwLock<MovieProgress>>) {
    ...
    bar.set_style(ProgressStyle::default_spinner().template("{msg} {spinner:.green}").unwrap());
    ...
    bar.set_style(ProgressStyle::with_template("{msg} [{bar:40.green/cyan}] frame {pos}/{len} ({percent}%)").unwrap());
