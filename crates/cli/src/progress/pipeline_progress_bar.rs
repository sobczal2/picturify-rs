...
pub fn run_progress_bar_for_pipeline(pipeline_progress: Arc<RwLock<PipelineProgress>>) {
    ...
    bar.set_style(
        ProgressStyle::with_template("{msg} [{bar:40.green/cyan}] {percent}%").unwrap(),
    );
    ...
    processor_names.iter().for_each(|name| {
        loop {
            let percentage = pipeline_progress.read().unwrap().get_individual_percentage(name.clone());
