...
pub fn read_image(args: ArgMatches) -> CliPicturifyResult<FastImage> {
    let input = args.get_one::<String>(ArgType::Input.to_id()).ok_or(CliPicturifyError::MissingArgument("Input".to_string()))?;
...
pub fn write_image(image: FastImage, args: ArgMatches) -> CliPicturifyResult<()> {
    let output = args.get_one::<String>(ArgType::Output.to_id()).ok_or(CliPicturifyError::MissingArgument("Output".to_string()))?;
