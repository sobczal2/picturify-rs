...
impl ApplyFnToPalettePixels for FastImage {
    fn apply_fn_to_linsrgba<F>(&mut self, f: F) where F: Fn(LinSrgba, usize, usize) -> LinSrgba {
    ...
    fn par_apply_fn_to_linsrgba<F>(&mut self, f: F) where F: Fn(LinSrgba, usize, usize) -> LinSrgba + Send + Sync {
        let _ = &self.inner.enumerate_rows_mut().par_bridge().for_each(|(_, row)| {
            row.into_iter().for_each(|(x, y, pixel)| {
                run_on_linsrgba_pixel(pixel, x as usize, y as usize, &f);
            });
        });
...
fn run_on_linsrgba_pixel<F>(
    pixel: &mut Rgba<f32>,
    x: usize,
    y: usize,
    f: F,
)
    where F: Fn(LinSrgba, usize, usize) -> LinSrgba
{
...
impl ApplyFnToImagePixels for FastImage {
    fn apply_fn_to_pixel<F>(&mut self, f: F) where F: Fn(&mut Rgba<f32>, usize, usize) {
        let _ = &self.inner.enumerate_rows_mut().for_each(|(_, row)| {
            row.into_iter().for_each(|(x, y, pixel)| {
    ...
    fn par_apply_fn_to_pixel<F>(&mut self, f: F) where F: Fn(&mut Rgba<f32>, usize, usize) + Send + Sync {
        let _ = &self.inner.enumerate_rows_mut().par_bridge().for_each(|(_, row)| {
            row.into_iter().for_each(|(x, y, pixel)| {
                f(pixel, x as usize, y as usize);
            });
...
impl ReadFromFile for FastImage {
    fn read_from_file(path: &str) -> PicturifyResult<Box<Self>> {
        ...
        Ok(Box::new(FastImage { inner: dynamic_image.into_rgba32f() }))
    }
}
